                ...
                <div class="card">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="body">
                                <h6 class="d-flex"><span class="mr-2">Name:</span><strong>{{ $user->first_name }} {{ $user->last_name }}</strong></h6>
                                <h6 class="d-flex"><span class="mr-2">Email:</span><strong>{{ $user->email }}</strong></h6>
                                ...
                                <tbody>
                                    @foreach ($user->invoices as $invoice)
                                        @if ($invoice->financingRequest)
                                            @can('view', $invoice->financingRequest)
                                                <tr>
                                                    <td>{{ $invoice->invoice_id }}</td>
                                                    <td>{{ number_format($invoice->calculateTotalAmount()) }}</td>
                                                    <td>{{ $invoice->financingRequest->created_at->format('d M Y') }}</td>
                                                    <td><span class="badge {{ $invoice->financingRequest->resolveStatus() }}">{{ Str::title($invoice->financingRequest->status) }}</span></td>
                                                    <td>
                                                        <a href="{{ route('financing.requests.show', ['financing_request' => $invoice->financingRequest]) }}" class="btn btn-sm btn-primary btn-round waves-effect">View</a>
                                                    </td>
                                                </tr>
                                            @endcan
                                        @endif
