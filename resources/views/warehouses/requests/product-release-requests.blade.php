                            ...
                            <tbody>
                                @foreach ($orders as $warehouse_order)
                                    <tr>
                                        ...
                                        <td>{{ $warehouse_order->warehouse->name }}</td>
                                        <td><span class="badge {{ $warehouse_order->resolveBadgeStatus() }}">{{ Str::title($warehouse_order->status) }}</span></td>
                                        <td>{{ $warehouse_order->created_at->format('d M Y') }}</td>
                                        <td>
                                            <a href="{{ route('orders.show', ['order' => $warehouse_order->orderItem->order]) }}" class="btn btn-sm btn-primary btn-round waves-effect">Details</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
